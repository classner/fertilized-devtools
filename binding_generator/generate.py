...
PYINTERFACE_DIR = os.path.join('..', '..', 'bindings', 'python')
MATINTERFACE_DIR = os.path.join('..', '..', 'bindings', 'matlab')
...
for child_str, parent_str in using_rel_strs:
  ...
  if parent_str not in defined_class_assocs:
    raise Exception(("During parsing, I determined that class {} is using " +\
     ...
     "header names. If you are using an external header, simply blacklist " +\
     "it in the file CodeGenerator/SingleFileParser.py (list starting on " +\
     "line 46).").format(parent_str, child_str, parent_str))
...
fileList = []
rootdir = os.path.abspath(os.path.join(BASE_DIR, 'ndarray'))
...
with open(os.path.join(BASE_DIR, 'fertilized.h'),'w') as fhfile:
...
with open(os.path.join(BASE_DIR, 'fertilized_fwd.h'), 'w') as fwdfile:
...
with open(os.path.join(BASE_DIR, 'soil.h'), 'w') as soilfile:
  soilfile.write(generated)

print('Generating serialization classes...')
# Cleanup!
delfiles = glob.glob(os.path.join(BASE_OUT_DIR, 'serialization', '__*'))
...
if not os.path.exists(os.path.join(PYINTERFACE_DIR, 'fertilized')):
    os.mkdir(os.path.join(PYINTERFACE_DIR, 'fertilized'))
shutil.copyfile('TypeTranslations.py', os.path.join(PYINTERFACE_DIR,
                                                    'fertilized',
                                                    'TypeTranslations.py'))

print('Generating python exporters...')
if not os.path.exists(os.path.join(PYINTERFACE_DIR, 'exporters')):
    os.mkdir(os.path.join(PYINTERFACE_DIR, 'exporters'))
else:
    # Cleanup!
    delfiles = glob.glob(os.path.join(PYINTERFACE_DIR, 'exporters', '__*'))
    for delfile in delfiles:
        os.remove(delfile)
...
pyfunc_export_dir = os.path.join(PYINTERFACE_DIR, 'exporters_mod_funcs')
def chunks(seq, size):
    r"""
...
for export_id, clsids in enumerate(chunks(list(range(len(cls_inst_tpls))), chunksize)):
    ...
    with open(os.path.join(PYINTERFACE_DIR,
...
with open(os.path.join(PYINTERFACE_DIR,
  ...
  expfile.write(generated)
vec_export_dir = os.path.join(PYINTERFACE_DIR, 'exporters_vec_types')
...
for tpind, vec_type in enumerate(vec_types):
    generated = array_exporter_tmpl.render({'vec_type':vec_type,
                                            'tpind':tpind,
                                            'header':vec_headers[vec_type].replace('/./', '/').replace('../../include/fertilized/', 'fertilized/')})
...
if not os.path.exists(os.path.join(MATINTERFACE_DIR, 'exporters')):
    os.mkdir(os.path.join(MATINTERFACE_DIR, 'exporters'))
else:
    # Cleanup!
    delfiles = glob.glob(os.path.join(MATINTERFACE_DIR, 'exporters', '__*'))
    for delfile in delfiles:
        os.remove(delfile)
if not os.path.exists(os.path.join(MATINTERFACE_DIR, 'fertilized', 'generated')):
    os.mkdir(os.path.join(MATINTERFACE_DIR, 'fertilized', 'generated'))
else:
    # Cleanup!
    delfiles = glob.glob(os.path.join(MATINTERFACE_DIR, 'fertilized', 'generated', '*'))
    for delfile in delfiles:
        os.remove(delfile)
if os.name == 'nt':
...
for key, group in itertools.groupby(classesGrouped, key=operator.attrgetter('Package')):
    ...
    try:
      ...
      with open(os.path.join(MATINTERFACE_DIR, 'matlab_headers', '__' + key + '.h'), 'w') as f:
          f.write(outputText)
    ...
    for cls in grouplist:
      ...
      with open(os.path.join(MATINTERFACE_DIR, 'fertilized', 'generated', cls.ClassName + '.m'), 'w') as f:
        f.write(outputText)
      if cls.ClassType.TemplateParams is None or len(cls.ClassType.TemplateParams) == 0:
        try:
          ...
          with open(os.path.join(MATINTERFACE_DIR, 'exporters', '__' + cls.ClassName + '.cpp'), 'w') as f:
            f.write(outputText)
        except Exception as ex:
        ...
        for instantiation_types in cls.SupportedTypes:
          try:
            ...
            outputText = cpp_tmpl.render( templateVars )
            with open(os.path.join(MATINTERFACE_DIR, 'exporters', '__' + cls.ClassName + '_' + str(inst_counter) + '.cpp'), 'w') as f:
              f.write(outputText)
          except Exception as ex:
...
for key, group in itertools.groupby(fcsGrouped, key=operator.attrgetter('DefiningHeader')):
    ...
    try:
      ...
      with open(os.path.join(MATINTERFACE_DIR, 'matlab_headers', '__functions_' + pkg + '.h'), 'w') as f:
        f.write(outputText)
    ...
    for func in function_list:
      try:
        ...
        with open(os.path.join(MATINTERFACE_DIR, 'exporters', '__functions_' + pkg + '_' + str(funcidx) + '.cpp'), 'w') as f:
          f.write(outputText)
      except Exception as ex:
...
with open(os.path.join(MATINTERFACE_DIR, 'matlab_headers', 'matlab.h'), 'w') as f:
    f.write(outputText)
